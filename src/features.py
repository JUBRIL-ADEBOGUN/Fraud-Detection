...
# from sklearn.preprocessing import OrdinalEncoder, OneHotEncoder # pyright: ignore
...
class XentePreprocessor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if 'TransactionStartTime' in X_tf.columns: 
            X_tf['TransactionStartTime'] = pd.to_datetime(X_tf['TransactionStartTime'],)
        ...
        if 'CustomerId' in X_tf.columns:
            X_tf['Customer_Avg_Amount'] = X_tf['CustomerId'].map(self.customer_avg_amount).fillna(self.global_avg_amount)
            X_tf['Customer_Transaction_Count'] = X_tf['CustomerId'].map(self.customer_tx_count).fillna(0)
            X_tf['Amount_to_Customer_Avg_Ratio'] = X_tf['Amount'] / (X_tf['Customer_Avg_Amount'] + 1e-5)
            
        if 'ProviderId' in X_tf.columns:
            X_tf['Provider_Transaction_Count'] = X_tf['ProviderId'].map(self.provider_tx_count).fillna(0)
        ...
        existing_cats = [c for c in categorical_cols if c in X_tf.columns]
        # onehot = OneHotEncoder(handle_unknown='ignore', sparse_output=False)
        # if existing_cats:
        #     onehot_features = onehot.fit_transform(X_tf[existing_cats])
        #     onehot_df = pd.DataFrame(onehot_features, columns=onehot.get_feature_names_out(existing_cats), index=X_tf.index)
        #     X_tf = pd.concat([X_tf, onehot_df], axis=1)
        #     X_tf = X_tf.drop(columns=existing_cats)
        
        # 5. Drop Useless Columns
        cols_to_drop = [
            'TransactionId', 'BatchId', 'AccountId', 'SubscriptionId', 'CustomerId',
            'CurrencyCode', 'CountryCode', 'TransactionStartTime', 'Amount',
        ]
        X_tf = X_tf.drop(columns=[c for c in cols_to_drop if c in X_tf.columns], errors='ignore')
        ...
        expected_columns = [
            ...
            'Value', 'PricingStrategy', 'Hour', 'DayOfWeek', 'DayOfMonth', 
            'Customer_Avg_Amount', 'Customer_Transaction_Count', 
            'Amount_to_Customer_Avg_Ratio', 'Provider_Transaction_Count', 'IsCredit'
        ]
